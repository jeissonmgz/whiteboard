...
export class ViewBoxDirective {
  ...
  constructor(private svg: ElementRef, private viewBoxService: ViewBoxService, private renderer2: Renderer2) {
    this.subscription = this.viewBoxService.getChanges().subscribe(
      change => {
        if (change.scroll){
          this.scroll(change.scroll.orientation, change.scroll.direction, change.scroll.value);
        }
      }
    );
  }
...
onResize(event) {
  ...
  applyScroll(isHorizontal:boolean, isTopOrLeft: boolean, value: number) {
    value = value/100;
    if (isHorizontal) {
      this.positionActual.x+= (isTopOrLeft?-value:value) * this.svg.nativeElement.width.baseVal.value;
    } else  {
      this.positionActual.y+= (isTopOrLeft?-value:value) * this.svg.nativeElement.height.baseVal.value;;
    }
  }
