import { RectShape, TypeShape, PropertyAllowed, ControlHandle, EditState, Point } from '../../../types/shape';
...
export class RectStrategy implements ShapeStrategy<RectShape> {
  public readonly type = TypeShape.RECT;

  public getPropertiesAllowed(): PropertyAllowed[] {
    return [PropertyAllowed.background, PropertyAllowed.line];
  }

  public getCenter(shape: RectShape): Point {
    return {
  ...
  public getBoundingBox(shape: RectShape): BoundingBox {
    return {
      x: shape.x,
      y: shape.y,
      width: shape.width,
  ...
  public generateControlHandles(shape: RectShape): ControlHandle[] {
  ...
  public createShape(id: string, startPoint: Point): RectShape {
    return {
      id,
      type: TypeShape.RECT,
      x: startPoint.x,
